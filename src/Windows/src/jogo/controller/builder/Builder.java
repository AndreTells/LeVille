...
import jogo.controller.gamecontroller.IControllerBuilder;
...
public class Builder implements IGameBuilder{
	...
	private IControllerBuilder controller_manager;
	...
	public void buildGame() {
		...
		player.connect(model.getBoardPlayer());
		...
		IActor controller = controller_manager.connectTurnController(player, board_model, stats_view, board_view_manager, event_manager);
		...
		for(int i=0;i<map_length;i++) {
			for(int j =0;j<map_height;j++) {
				IActor cell_controller = controller_manager.connectCellController(i, j);
